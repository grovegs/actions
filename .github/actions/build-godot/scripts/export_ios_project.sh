...
Debug)
    ...
    godot --path "${project_dir}" --rendering-driver opengl3 --export-debug "${preset}" "${export_file}" ||
...
Release)
    ...
    godot --path "${project_dir}" --rendering-driver opengl3 --export-release "${preset}" "${export_file}" ||
