...
Debug)
    ...
    if ! godot --path "${project_dir}" --headless --export-debug "${preset}" "${file}" >/dev/null 2>error.log; then
        echo "Error: Failed to export debug."
        cat error.log
        exit 1
    fi
    ;;
Release)
    ...
    if ! godot --path "${project_dir}" --headless --export-release "${preset}" "${file}" >/dev/null 2>error.log; then
        echo "Error: Failed to export release."
        cat error.log
        exit 1
    fi
    ;;
