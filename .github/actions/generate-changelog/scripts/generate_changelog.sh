...
categories=("Features" "Bug Fixes" "Hotfixes" "Chores" "Documentation" "Refactors" "Styling" "Performance" "Tests" "Build" "CI/CD" "Reverts" "Other")
commits_by_category=()
...
while IFS= read -r commit; do
    ...
    hotfix) category_index=2 ;;
    chore) category_index=3 ;;
    docs) category_index=4 ;;
    refactor) category_index=5 ;;
    style) category_index=6 ;;
    perf) category_index=7 ;;
    test) category_index=8 ;;
    build) category_index=9 ;;
    ci) category_index=10 ;;
    revert) category_index=11 ;;
    *) category_index=12 ;;
    esac
...
for i in "${!categories[@]}"; do
    if [ -n "${commits_by_category[$i]}" ]; then
        commits_by_category[i]=$(echo "${commits_by_category[i]}" | sort -t: -k1,1)
    fi
done

changelog=""
for i in "${!categories[@]}"; do
    category_name=${categories[$i]//_/ }
    if [ -n "${commits_by_category[$i]}" ]; then
