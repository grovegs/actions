...
if [[ "$RUNNER_OS" == "Windows" ]]; then
    unity_exe_path=$(find "C:/Program Files" -path "*/Editor/Unity.exe" -type f 2>/dev/null | grep -i "${unity_version}" | head -1)
...
if [[ "$RUNNER_OS" == "Windows" ]]; then
    echo "::notice::Creating unity.bat alias..."; echo "@echo off" > "${editor_dir}/unity.bat"; echo "\"${unity_exe_path}\" %*" >> "${editor_dir}/unity.bat"; chmod +x "${editor_dir}/unity.bat"
fi
